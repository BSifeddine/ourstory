...
export default function BelovedWifeCard() {
  ...
  const handleNoClick = () => {
  setAnswer('no');
  // Revert back to null (buttons visible) after 2000 milliseconds (2 seconds)
  setTimeout(() => {
    setAnswer(null);
  }, 2000);
}; 
  ...
  return (
    <div className="min-h-screen bg-pink-100 flex items-center justify-center p-4 font-serif text-pink-900">
      <div className="max-w-4xl w-full bg-white/40 backdrop-blur-lg rounded-3xl shadow-2xl p-8 flex flex-col md:flex-row gap-8 border border-white/50">
        
        {/* Sidebar Navigation */}
        <div className="flex md:flex-col gap-6 justify-center md:justify-start pt-4">
          {stepsData.map((step) => (
            <div key={step.id} className="flex flex-col items-center gap-1 cursor-pointer" onClick={() => setCurrentStep(step.id)}>
              <div className={`p-2 rounded-full transition-all ${currentStep === step.id ? 'bg-pink-300 scale-110' : 'hover:bg-pink-200'}`}>
                🐻
              </div>
              <span className="text-xs font-bold text-pink-700">Step {step.id}</span>
            </div>
          ))}
        </div>
        ...
        <div className="flex-grow flex flex-col items-center text-center">
          <h1 className="text-3xl font-serif mb-6">To My Beloved Wife</h1>
          
          <div className="w-32 h-32 rounded-full border-4 border-pink-200 overflow-hidden mb-4 shadow-lg">
            <img src="/bears.jfif" alt="Bears" className="w-full h-full object-cover" />
          </div>

          <div className="bg-white/60 p-8 rounded-2xl shadow-inner max-w-lg">
            <h2 className="text-2xl font-semibold mb-4">{currentData.title}</h2>
            <p className="text-pink-800 leading-relaxed font-serif mb-6">{currentData.content}</p>
    ...
    {answer === null ? (
      <div className="flex gap-4">
        <button 
          ...
          className="bg-pink-500 text-white px-8 py-2 rounded-full hover:bg-pink-600 transition"
        >
          Yes
        </button>
        <button 
          onClick={handleNoClick} // Call the timeout function
          className="bg-gray-300 text-white px-8 py-2 rounded-full hover:bg-gray-400 transition"
        >
          No
        </button>
      </div>
    ) : answer === 'no' ? (
      <p className="text-xl font-bold text-red-600 animate-bounce">
        Say yes! 😡
      </p>
    ) : (
      <div className="fixed inset-0 bg-pink-100/90 flex items-center justify-center z-50 p-6 text-center">
          ...
          I love you more than everything in this world Habibti 💍❤️
            ...
            {currentStep < 7 && (
              <button 
                ...
                className="bg-pink-400 hover:bg-pink-500 text-white px-6 py-2 mt-2 rounded-full transition-all shadow-md"
              >
                Continue &gt;
              </button>
            )}
        ...
        {/* Right Info Section */}
        <div className="w-full md:w-48 border-t md:border-t-0 md:border-l border-pink-200 pl-0 md:pl-6 pt-6 md:pt-0 flex flex-col items-center text-center">
          <p className="text-xs uppercase tracking-widest mb-2 text-pink-500">Our Journey</p>
          ...
          <p className="font-bold">Sifeddine Bouaoua</p>
          <p className="italic mt-2 text-sm">Your Husband</p>
        </div>
      </div>
    </div>
  );
